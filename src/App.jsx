        ...
        {listing && (stage === "ready" || stage === "posted") && (
          <div className="mt-6 rounded-xl p-5" style={{ background: THEME.paper }}>
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              {stage === "ready" && (
                <>
                  <Button variant="leaf" icon={CheckCircle2} onClick={() => { setStage("posted"); onPost && onPost(listing); }}>
                    Post to marketplace
                  </Button>
        ...
        {filtered.map((l) => (
          <div key={l.id} className="flex flex-col rounded-2xl p-5" style={{ background: THEME.paper2 }}>
            ...
            <div className="mt-4 flex items-center justify-between">
              ...
              <Button variant="dark" icon={ShoppingCart} onClick={() => onBuy(l)}>Buy now</Button>
            </div>
          </div>
        ))}
        {filtered.length === 0 && (
...
function OrderTracking({ order, onBack }) {
  const [stepIndex, setStepIndex] = useState(order ? 0 : ORDER_STEPS.length - 1);
  const timers = useRef([]);
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-14">
      ...
      <h1 className="ks-display text-3xl font-semibold" style={{ color: THEME.ink }}>
        {order.emoji} {order.qty} kg {order.crop} from {order.farmer}
      </h1>
      <p className="mt-1 text-sm" style={{ color: THEME.ink, opacity: 0.7 }}>
        Total: <span className="ks-mono font-semibold">₹{order.qty * order.price}</span> · held in escrow until delivery is verified.
      </p>
      ...
      <div className="mt-8 rounded-2xl p-6" style={{ background: THEME.soil }}>
        ...
        <div className="mt-4 flex items-center gap-3">
          {!done ? (
            <Button variant="primary" icon={ChevronRight} onClick={advance}>
              Simulate next step
            </Button>
          ) : (
...
export default function App() {
  ...
  const handlePost = useCallback((listing) => {
    setListings((prev) => [
      {
        id: Date.now(),
        crop: listing.crop,
        emoji: "🍅",
        farmer: "You (new listing)",
        village: listing.village,
        qty: listing.qty,
        price: listing.suggestedPrice,
        distance: "0 km",
        freshness: "Just listed",
      },
      ...prev,
    ]);
  }, []);

  const handleBuy = useCallback((listing) => {
  ...
  return (
    <div className="ks-root min-h-screen" style={{ background: THEME.paper }}>
      ...
      {view === "marketplace" && <Marketplace listings={listings} onBuy={handleBuy} />}
